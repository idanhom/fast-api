...
class BookRequest(BaseModel):
    ...
    published_date: int = Field(ge=1900, le=2100)
...
@app.get("/books")
async def read_all_books(rating: Optional[int] = Query(default=None, ge=1, le=5),
                         published_date: Optional[int] = Query(default=None, ge=1900, le=2100)):
    filtered_books = books
    if rating is not None:
        filtered_books = [book for book in filtered_books if book.rating == rating]
    if published_date is not None:
        filtered_books = [book for book in filtered_books if book.published_date == published_date]
